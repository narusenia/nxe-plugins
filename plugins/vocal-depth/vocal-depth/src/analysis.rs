...
use nxe_dsp::Handoff;
use vocal_depth_core::reflections::TAPS;
...
pub struct Analysis {
    ...
    /// the ratio *is* the distance (`REQ-VDP-018`), and two numbers say which
    ...
    /// changed the window (`vocal_depth_core::Reflections::pattern`).
    ...
    /// (`REQ-VDP-006`): if the plugin is quietly undoing part of the distance
    /// the user asked for, the window says so and says how far.
    pub clarity: Handoff<1>,
    /// How alike the reflections' two channels are.
    ///
    /// **The width promise, measured** (`REQ-VDP-007`): the two channels share
    ...
    /// be a lie (`vocal_depth_core::Damping::direct_corner_hz`).
