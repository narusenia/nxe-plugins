...
//!
//! **The reflections widen; the direct sound does not move** (`REQ-VDP-007`).
...
//! made here. What replaces it (`REQ-VDP-007`):
//!
...
//!
//! Specified in `plugins/vocal-depth/docs/specifications/dsp.md`, "WIDTH".
...
/// `ulp(value) / coefficient` a one-pole stops changing the sum (`VDP-1`).
...
impl Width {
    ...
    /// material: the two channels share no tap time, and `VDP-1` measured
    ...
    /// The direct sound's side gain, for the display (`REQ-VDP-018`).
...
mod tests {
    ...
    /// orthogonal. This is the whole of `REQ-VDP-007`'s promise about the direct
    ...
    /// **Zero leaves the two channels identical** (`REQ-VDP-011`).
    #[test]
    ...
    /// the other way — narrow when close (`REQ-VDP-002`'s table).
    ///
    ...
    fn the_power_factor_matches_the_operation() {
        ...
        // (`VDP-6`). Reversing one channel decorrelates it (`AIR-1` does the
        // same).
    ...
    /// 2.5 dB, which read as "no comb" (`VDP-6`).
    ///
    ...
    /// (`REQ-VDP-007`), measured on the pair that actually ships: the tap sets
    ...
    /// changing — narrowing the direct sound is in `REQ-VDP-002`'s table, and a
