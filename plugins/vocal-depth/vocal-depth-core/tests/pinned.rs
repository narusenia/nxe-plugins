//! What `VDP-8` pins: block size, sample rate, hostile values and seams.
//!
...
/// **The output does not depend on the host's buffer size** (`REQ-VDP-017`).
///
...
/// Every rate produces a working, finite, comparable engine (`REQ-VDP-017`).
///
...
/// (`REQ-VDP-016`). Nothing non-finite comes out of the wet path and nothing
/// panics.
#[test]
...
/// **A parameter step does not click** (`REQ-VDP-016`).
///
...
/// that size (`VDP-2` found that snapping a *retuned* filter produces no seam
...
fn a_step_in_every_macro_leaves_no_seam() {
    ...
    // zero crossing has nothing to multiply (`VDP-2`).
    let step_at = 12_007;
