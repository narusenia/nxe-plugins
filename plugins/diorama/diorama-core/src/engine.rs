...
//! (`REQ-DIO-001`), which is the whole of the `MIX` = 0 bit-identity: the
...
//!
//! Specified in `plugins/diorama/docs/specifications/dsp.md`.
//!
...
//! signal (`REQ-DIO-008`). What keeps that honest is that it does nothing on
...
/// gain that steps at block rate is a seam (`DIO-1`).
///
...
/// last makes the first five milliseconds wet (`DIO-4` found this through the
...
impl Engine {
    ...
    pub fn process(&mut self, left: f32, right: f32) -> (f32, f32) {
        // The dry, branched before anything (`REQ-DIO-001`).
        let dry = (left, right);

        // **Linked across the pair** (`REQ-DIO-011`): one reading off the mono
        ...
        // event that sharpens the band (`REQ-DIO-005`).
        ...
        // the promise is bit-identity (`REQ-DIO-001`) and a denormal in the wet
    ...
    /// **A function of the parameters alone** (`REQ-DIO-008`).
    ...
    /// Where each reflection arrives and how loud it is (`REQ-DIO-013`).
    ...
    /// How far open the transient detector is (`REQ-DIO-018`).
    ...
    /// The two damping corners, for the display (`REQ-DIO-018`). `None` on a
...
mod tests {
    ...
    /// above 8 kHz. **It barely changes the answer** (`DIO-5` measured 1.26
    ...
    /// `REQ-DIO-008`'s `±1.0 dB` is a 2.0 dB window and therefore a spread of
    ...
    /// missing the *original* `±0.5 dB` by 0.2 (`DIO-7`).
    ...
    /// **The gate** (`REQ-DIO-008`): sweeping `DEPTH` may not move the level.
    ///
    /// **The bound is 1.0 dB, and it used to be 0.5.** `DIO-3` measured that no
    ...
    /// gain forbidden from looking at the signal (`REQ-DIO-008`) is right for
    ...
    ///
    /// **The materials are the ones `REQ-DIO-008` names — pink noise and a
    ...
    ///
    /// **`DIO-14` spent most of the margin on purpose.** A listener reported
    ...
    /// not look at the signal (`REQ-DIO-008`) has to pick one.
    ///
    ...
    fn white_noise_is_outside_what_the_normalisation_can_hold() {
        ...
        // **7.6 dB after `DIO-14`**, up from 4.4: widening the damping range
    ...
    /// **The gate holds with `DAMPING` open too** (`REQ-DIO-005`'s last
    ...
    /// `ROOM` is an amount, not a level (`REQ-DIO-008`).
    #[test]
    fn room_does_not_move_the_loudness() {
        let signal = harmonics::pink(0.3, 2 * RATE as usize);
        // Measured **1.48 dB peak to peak** (= ±0.74) after `DIO-14` widened
    ...
    fn the_sweep_moves_the_level_without_the_normalisation() {
        ...
        // normalisation in (`DIO-3`) — a factor of nine.
        assert!(
            spread > 1.0,
    ...
    /// (`REQ-DIO-008`). Four materials, one number.
    #[test]
    ...
    /// plugin in the line keeps (`REQ-DIO-001`).
    #[test]
    fn mix_zero_is_bit_identical() {
        ...
        // which is what a session saved with `MIX` = 0 needs (`DIO-4`).
    ...
    fn hostile_values_stay_finite() {
        ...
        // the output (`REQ-DIO-001`), so a host that sends a NaN gets one back
