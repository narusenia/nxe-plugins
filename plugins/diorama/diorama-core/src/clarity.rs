...
//!
//! **The guard, inverted** (`REQ-DIO-006`). `nxe_audio::guard::RelativeGuard`
...
//!
//! Specified in `plugins/diorama/docs/specifications/dsp.md`, "CLARITY".
...
/// deficit.
///
/// **Measured, not chosen** (`DIO-7`, and `SPK-18` is why it is measured from a
...
/// straight into the presence band — `DIO-8` saw it as 20 times the background
...
impl Clarity {
    ...
    /// mechanism nobody can switch off is one nobody can hear (`REQ-DIO-006`),
    ...
    ///
    /// Detection is linked across the pair (`REQ-DIO-011`): one reading drives
    ...
    /// works invisibly is a control that does nothing (`REQ-DIO-006`,
    /// `.agents/rules/ui.md`).
...
mod tests {
    ...
    /// which is exactly what `DIO-3` measured and why
    ...
    /// **Where the threshold came from** (`DIO-7`). The ratio is
    ...
    /// **Zero is exactly nothing**, at either end (`REQ-DIO-006`).
    #[test]
    ...
    /// The lift grows with `DEPTH` (`REQ-DIO-006`).
    #[test]
    ...
    /// **A ratio cannot depend on the input level** (`REQ-DIO-006`), which is
