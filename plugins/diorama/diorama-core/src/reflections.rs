...
//!
//! **This module does not know what Diorama is** (`REQ-DIO-003`). It takes
...
//! reflections — Diorama, Vocal Glue's `Ambience Glue`, Impact's `SIZE` —
//! and the second one to ask moves this file into `nxe-audio` unchanged
//! (`REQ-DIO-015`).
//!
//! The design is specified in
//! `plugins/diorama/docs/specifications/dsp.md`. Three points from it are
...
//!
//! What is *not* here: `DAMPING` (`DIO-5`) and the stereo width
//! (`DIO-6`). Both act on the reflection bus as a whole, so the engine owns
...
/// costs no phase rotation (`REQ-DIO-007`).
...
/// **-4.6 dB of the energy beyond 120 ms** (`DIO-1`). What sets the extent is
...
/// not (`REQ-DIO-002`).
///
/// **The far end is above unity, and it has to be** (`DIO-14`). What says "far"
...
/// silent at zero" (`REQ-DIO-003`); a power keeps that exact and continuous.
...
/// (`REQ-DIO-003`) does not hold without a floor. nih-plug enables FTZ around
...
/// times the background roughness before this existed (`DIO-1`). Smoothing
...
/// runs on every sample for ever (`DIO-1`). This is 70 dB below the weights it
...
impl Channel {
    ...
    /// (`DIO-3`). Exposed rather than recomputed there so the tap table has
    /// one owner.
    ///
...
impl Reflections {
    ...
    /// (`DIO-3`). Exactly zero when `amount` is zero.
    ...
    /// loudness normalisation needs (`DIO-3`). **The bus gain is already in
...
///
/// **Times are milliseconds and lengths are samples** (`REQ-DIO-017`): holding
...
mod tests {
    ...
    /// (`REQ-DIO-003`). A `-120 dB` bus is still a bus.
    #[test]
    ...
    /// lives inside the range `REQ-DIO-003` names. **The share beyond 120 ms
    ...
    fn the_response_starts_at_the_first_tap_and_does_not_become_a_tail() {
        ...
        // Measured -25.5 dB and -140.7 dB (`DIO-1`, recorded in `dsp.md`).
    ...
    fn sweeping_distance_does_not_click() {
            ...
            for (index, &sample) in tone.iter().enumerate() {
                if index == step_at {
                    ...
                    if snap {
                        ...
                        // out, which is what the module did before `DIO-1`
                        // measured it.
        ...
        // Measured 0.36 against a control of 88 (`DIO-1`) — the smoothed move
    ...
    /// (`REQ-DIO-003`). A first-order tail settles on a denormal instead of on
    ...
    fn silence_converges_to_exact_zero() {
        ...
        // Measured 0.150 s against a budget of three (`DIO-1`).
        assert!(
    ...
    /// 96 kHz from halving the room (`REQ-DIO-017`).
    #[test]
    ...
    fn the_response_has_the_same_shape_at_every_rate() {
        ...
        for (rate, measured) in RATES.iter().zip(&energies) {
            ...
            // Measured within 0.08 dB over the four rates (`DIO-1`).
            assert!(
                difference.abs() < 0.5,
    ...
    /// exposed tap energy is what the normalisation in `DIO-3` will square.
    #[test]
    ...
    /// without a single phase rotation (`REQ-DIO-007`). `DIO-6` owns the
    ...
    fn the_two_channels_decorrelate() {
        ...
        // Measured -0.006 (`DIO-1`): the two tap sets share no time, so this
    ...
    /// (`REQ-DIO-016`). A single NaN sample used to be enough to stop a whole
    /// plugin (`SPK-9`).
    #[test]
