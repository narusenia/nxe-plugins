...
//!
//! **This is the product's claim** (`REQ-DIO-008`): turning `DEPTH` moves the
...
//! "ラウドネス正規化".
//!
//! **The normalisation never looks at the signal** (`REQ-DIO-008`). It resolves
...
/// combination of hostile values turns into an infinity (`REQ-DIO-016`).
...
/// out.
///
/// **This is one number with a decision behind it, and `REQ-DIO-008` cannot be
...
/// white, and **1.2 %** of a sparse harmonic phrase (`DIO-3`). A gain that may
/// not look at the signal (`REQ-DIO-008`) has to pick one of them.
///
/// What each choice costs, measured as the spread of output RMS across the whole
/// of `DEPTH` (`DIO-3`, the gate asks for 0.5 dB):
///
...
///
/// **Re-tuned in `DIO-14`**, after the distance cue moved to the direct sound's
...
///
/// **Measured the same way and it has the same wall** (`DIO-5`). What a lowpass
...
pub struct Macros {
    ...
    /// where it is (`REQ-DIO-007`).
    pub width: f32,
    /// How much intelligibility to put back. Exactly none at zero, and none at
    /// all when the voice is close (`REQ-DIO-006`).
    pub clarity: f32,
    /// Dry to wet. **Exactly dry at zero, bit for bit** (`REQ-DIO-001`).
    pub mix: f32,
...
impl Probe {
    ...
    /// the signal and this may not (`REQ-DIO-008`): the transient's few
    /// hundredths of a dB (`DIO-2` measured 0.045 dB on steady material) and
    /// what `CLARITY` puts back (`DIO-7`).
    ///
    ...
    pub fn gain(&self, resolved: Resolved, sample_rate: f32) -> f32 {
        ...
        // through — bounded, and measured in `DIO-2`.
        let direct_damping =
