//! Nothing on the audio path may allocate (`REQ-DIO-016`).
//!
...
#[test]
fn neither_processing_nor_setting_the_macros_allocates() {
    // Building is the only place that may (`Engine::new`, where the delay lines
    // come from), and it happens before anyone is watching.
    ...
    for (block, chunk) in input.chunks(BLOCK).enumerate() {
        // **Moved every block**, so what is being watched is the paths that
        // rebuild coefficients and tap weights rather than the early returns.
        ...
        engine.set(Macros {
            ...
            width: 1.0 - position,
            clarity: position,
            mix: 0.5 + position * 0.5,
            output: 1.0,
        });
        for sample in chunk {
            std::hint::black_box(engine.process(*sample, *sample * 0.5));
        }
    }
