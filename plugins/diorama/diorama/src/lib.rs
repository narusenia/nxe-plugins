//! The NXE Diorama nih-plug wrapper: parameter declarations, and the
//! wiring between them and `diorama_core`.
//!
//! No DSP lives here (`.agents/rules/rust.md`).
//!
//! The window is `ui`. **One screen, no tabs** (`REQ-DIO-013`).
//!
//! **The product name is settled** (`REQ-DIO-014`, `DIO-15`). It was
//! `NXE Vocal Depth` — a descriptive name in a line that otherwise uses
//! invented ones — and then `NXE Parallax`, which was dropped because five
//! others in this market already use it. `CLAP_ID` is written into a host's
//! project file, so this was the last moment it was free to move.
...
impl Default for Diorama {
    fn default() -> Self {
        Self {
            params: Arc::new(DioramaParams::default()),
...
impl Plugin for Diorama {
    const NAME: &'static str = "NXE Diorama";
    ...
    const AUDIO_IO_LAYOUTS: &'static [AudioIOLayout] = &[
        ...
        // **Mono in, stereo out** (`REQ-DIO-011`). The reflections are stereo by
    ...
    /// the rate the host has just committed to (`REQ-DIO-016`).
    ///
    ...
    /// no bulk delay in it (`REQ-DIO-012`).
    fn initialize(
        &mut self,
    ...
    fn process(
        ...
        for sample in 0..samples {
            ...
            // first instead of reading it (`REQ-DIO-011`).
        ...
        //
        // **Reading, never writing** (`REQ-DIO-018`): everything here comes out
...
mod tests {
    ...
    fn render(plugin: &mut Diorama, left_in: &[f32], right_in: &[f32]) -> (Vec<f32>, Vec<f32>) {
        plugin
            .engine
    ...
    /// an asleep smoother would show up (`REQ-DIO-001`).
    #[test]
    ...
    /// clamping has to survive the trip (`REQ-DIO-016`).
    #[test]
