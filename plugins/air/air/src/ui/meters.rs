...
//!
//! **The judgement an additive plugin is used for** is "is this better, or just
//! louder" (`.agents/rules/ui.md`). Two pairs of bars on the same scale is what
//! makes that answerable without reaching for the host's meters.
//!
//! **No protection meter.** The strip above says how far it is pulling, and the
//! figure sinks when it does — the same number in three places is two of them
//! being ignored.
...
///
/// **Air does move the image** — that is what `WIDTH` is for — so two bars that
/// differ at the output and agree at the input is the layer's own spread being
/// read (`REQ-AIR-008`).
