...
///
/// **`reduction` is zero until `VEL-15`** wires the analysis. What it will carry
/// is how far each guard is pulling, which is the whole point of the figure
/// sinking when the plugin protects you (`REQ-VEL-006`).
fn bands_of(params: &VelourParams, host_rate: f32) -> Vec<Band> {
...
pub fn view(cx: &mut Context, host_rate: f32) {
    ...
    VStack::new(cx, |cx| {
        BandField::new(
            cx,
            Ui::params.map(move |params| bands_of(params, host_rate)),
            // The two analysis layers arrive in `VEL-15`. Empty until then: the
            // widget draws nothing for an empty curve, which is also what an
            // idle track looks like.
            Ui::dry,
            Ui::wet,
        ...
        .focus(super::param_bind::value_of(Ui::params, |params| &params.focus))
        .height(Stretch(1.0))
        .width(Stretch(1.0));
...
mod tests {
    ...
    fn the_regions_are_in_the_engine_order() {
        ...
        let bands = bands_of(&params, 48_000.0);
    ...
    fn focus_slides_every_region() {
        let params = VelourParams::default();
        let resting = bands_of(&params, 48_000.0);
