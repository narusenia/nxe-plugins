...
//!
//! **The judgement a dynamics processor is used for** is "is this better, or
//! just louder" (`ui.md`). Two pairs of bars on the same scale is what makes
//! that answerable without reaching for the host's meters.
//!
//! **No reduction meter.** What the bands are doing is already on screen — the
//! regions move by the amount they are being moved (`SPK-13`) — and the same
...
/// information — Sparkleur does not move the image (`REQ-SPK-011`), so two bars
