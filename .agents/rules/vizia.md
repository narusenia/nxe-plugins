...
**`.class("root")` を根に付け忘れると、窓の地はホストの黒（0x00）のまま。**
`.panel` は `BACKGROUND`（0x0A）なので、**パネルだけが明るい箱に見える**。
テーマの「面は 2 段階で、分けるのは 1 px の枠だけ」は**窓がその 1 段目である
ことが前提**なので、根が塗られていないと設計が成立しない。

Sparkleur は最初からこれが抜けていて、隣に大きな黒い図がある小窓で初めて
気付いた。**スクリーンショットの画素を測る**のが一番速い切り分けだった
（パネル 0x0A / 窓 0x00 とはっきり出る）。
