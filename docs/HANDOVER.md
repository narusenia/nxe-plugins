...
**2026-08-27 時点。3 つとも `v0.1.1` で公開済み。テスト 447 本。**

**Air は `AIR-7` まで通っていて、音が出る。** `mise run bundle air` で
CLAP と VST3 が出る。DSP は全部入っている（2 系統の生成器、`WIDTH`、
Follow 3 本、保護）。**残っているのは UI（`AIR-8`〜`AIR-11`）と CPU 予算と
既定値。** 次にやるのは `ui.md` を書くこと。詳しくは「次にやること」。
...
| `plugins/air/air` | NXE Air。CLAP + VST3。パラメータ 15 個。**UI がまだ無い**（`AIR-8`〜） |
| `plugins/air/docs` | 要件・DSP 仕様・実装計画。**UI 仕様だけ未作成**（`AIR-8` の前） |
...
### 1. Air の `ui.md` を書く（`AIR-8` の前に）

**ウィジェットは決まっている**（`air-plan.md` の `AIR-9`）: **点のスペクトラム**。
原音は細い線、生成層は点の格子で、レベルが上がるほど点が密になる。
`BLEND` を倍音側へ振ると点が縦に揃い、ノイズ側で散る — 2 系統の違いが
そのまま図になる。`nxe-ui` に新規で起こし、**同じ変更で `examples/gallery.rs`
にも入れる**（`.agents/rules/vizia.md`）。

決まっていないのは配置と、読み値に何を出すか。材料は既に全部ある:

- Follow の 3 係数（`Engine::follow_coefficients`）— **掛け算なので 1 本でも
  閉じていれば層は出ない。3 本とも出す**（`REQ-AIR-018`）
- 保護の引き量（`Engine::guard_reduction_db`）— **強く効くと層が消えるので、
  見せないと壊れていると読まれる**
- 生成層そのもの（`Engine::layer`）— 引き算ではなく直接取れる

### 2. `AIR-8`〜`AIR-11` — UI

マクロ 7 本 → 点のスペクトラム → 読み値と Advanced → 解析の配線。
**パラメータは 15 個**で、うち 6 個が Advanced（`DRIVE` / `BIAS` /
Follow 3 本の偏差 / 保護の偏差）。

### 3. `AIR-12` / `AIR-13` — CPU 予算と既定値

**既定値は全部仮**。`AIR-13` は耳が要る唯一の単位で、`dsp.md` の
「耳で詰める定数」にどれが測定で決まっていてどれが耳かが書いてある。

### 実機で聴くなら（いつでも）

```bash
mise run bundle air     # CLAP と VST3 が target/bundled に出る
```

**UI はまだ無い**ので、ホストの既定パラメータ UI で 15 個のノブが出る。
`SURFACE` を 1 本回せば層が乗る。`BLEND` 0 が倍音、1 がノイズ。
