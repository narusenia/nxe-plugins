...
**まだ実機で 1 度も見ていない。** 窓は 720 × 546 で、寸法は算術で出しては
あるが、**`.agents/rules/ui.md` が「寸法はホストの中でしか決まらない。
最初に書いた数は間違っていると思え」と書いているとおり**、Velour は
580 → 528、Sparkleur の図は 236 → 176 → 200 と動いている。
**UI の欠陥はテストが 1 本も落ちない。**
...
- **継ぎ目の測り方を 3 回間違えた**（`AIR-7`）。ノイズが混ざっていると
  ...
  **4 回目でようやく対照実験が差を出した**
- **整形済みのファイルへの文字列置換が 3 回黙って失敗した。**
  `.agents/rules/rust.md` にそのまま書いてある形。1 回は
  **保護が配線されていないまま「全部 0.00 dB」という good に見える測定結果**が
  出た（`SPK-18` の「効くはずのものが配線されていない」と同じ）。
  **置換のあとは grep する**
