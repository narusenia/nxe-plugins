...
`velour-v0.1.0` で**一区切り**（下書き Release）。**Sparkleur は `SPK-1`〜`SPK-18`
完了** — 測れるものは全部測って固定し、既定値は 1 つも動かす理由が出なかった。
**残っているのは主観のサインオフだけ**（下のチェックリスト。**リリースの
阻害要因ではない**）。
...
**3 つのプラグインの UI をもう少しリッチにする**（2026-08-27 に着手予定）。
`SPK-18` は閉じた。
