...
**2026-08-28 時点。4 本とも公開済み（`v0.1.3` / Air は `v0.1.2`）。テスト 474 本、
`mise run check` は通っている。**

**次は Vocal Depth の `VDP-1`（初期反射）。** 要件（`REQ-VDP.md`）、実装計画
（`vocal-depth-plan.md`）、**`dsp.md` まで書けている**（2026-08-28）—
**数字は 1 つも測っていない**ので、「耳で詰める定数」の表がどれを測って
どれを耳でやるかの正。詳しくは「次にやること」。
...
| `plugins/vocal-depth/docs` | **要件・DSP 仕様・実装計画。次に作るもの**（`VDP-1`〜`VDP-13`）。初期反射を作る本で、Glue と Impact に貸す。`ui.md` は `VDP-9` の前 |
...
| 問い | 答え |
|---|---|
| タップ時刻を `DEPTH` で動かすか | **動かさない。** 重み包絡（σ = 0.45 のガウス窓）の中心を動かす。補間もクリックも、掃いている間の detune も無い。**整数読みで済むので CPU も 4 分の 1** |
| タップの本数と時刻 | **13 本 × 2 チャネル。10〜120 ms の素数 ms を L/R 交互** |
| オールパス | **3 段、`g` = 0.55、タップ列の前**（入力拡散） |
...
- **`doubler_core::delay::DelayLine` を `nxe-audio` へ**（`VDP-1`）。
  **2 個目の客が要求した**ので規則どおり。整数読みを 1 本足す
