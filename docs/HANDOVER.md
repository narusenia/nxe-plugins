...
完了、`SPK-18` が進行中** — 測れるものは測って固定し、De-Harsh のしきい値と
参照帯を実測で決め直した。**残っているのは実素材での 1 周（耳）。**
...
**`SPK-18` の残りは実素材で 1 周すること。** `mise run install sparkleur` して
声・ベース・シンセ・ドラムバスに挿し、既定値が「挿しただけで良い」と言えるか。

**測れるものはもう測って固定してある**（`plugins/sparkleur/sparkleur/src/defaults.rs`）。
このファイルは**ラッパ側**にあり、`SparkleurParams::default()` から出荷時の
既定値そのものを読む — 測っている数と出荷する数がずれようがない。

| 済み | 実測 |
|---|---|
| 無音で呼吸しない | 出力ピーク 0、全帯域ゲイン 0、ゲート 0。`LIFT` を開けば −80 dBFS を持ち上げる |
| 普通の素材で両側が動く | ピンク −42 dBFS で **+0.96 dB**、−6 dBFS で **−1.36 dB**、−18 dBFS では **−0.03 dB** で素通り |
| ハットで光りパッドで光らない | ゲート ハット **1.000** / パッド **0.107**、平均で 14 倍 |
| 残留が −60 dB 以下 | 既定 **−68.3 dB**（CRUSH + `SPARK` 全開でも −52.3 dB） |
| De-Harsh が普通の素材を引かない | ピンクノイズで **ちょうど 0.00 dB**（直す前は 1.3 dB 引いていた） |
| 軸がラウドネスを動かさない | 持続音 4 素材で **0.13 dB 以内**。トリムは 0 のまま（下） |
| 境界が素材を分け合う | ピンクノイズの取り分が上 4 帯域で **1.6 dB 以内**。`FOCUS` は 3 オクターブ、3 レートとも交差なし |
...
| `sparkle::DRIVE` | 4.0 | **艶が足りるか。**折り返しの側は済み |
| 境界 4 点 | 120 / 400 / 1.5k / 6k | **どの帯域が「声に効く」か。**分け方の側は済み |

**`CHARACTER` の読み値の持ち越しは片付いた**（下の罠を参照）。
