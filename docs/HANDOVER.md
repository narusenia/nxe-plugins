...
**2026-08-27 時点。3 つとも `v0.1.1` で公開済み。テスト 461 本。**

**Air は `AIR-12` まで通っている。** DSP も窓も入っていて、
`mise run bundle air` で CLAP と VST3 が出る。エンジンは **47.2 µs**
（予算 533）。**残っているのは `AIR-13`（既定値）だけで、それは耳が要る。**

**実機で 1 回壊れていた**（`148e576` で修正済み）。`process` の解析
publish ブロックが丸ごと落ちていて、**テスト 461 本が全部通ったまま、窓は
「信号の来ていないトラック」と同じ絵で静止していた**。4 本とも
`analysis.rs` の `Handoff` を `lib.rs` の `write` と突き合わせるスキャン
テストを入れた。

**寸法はまだ実機で詰めていない。** 720 × 546 は算術で出した数で、
**`.agents/rules/ui.md` の「寸法はホストの中でしか決まらない。最初に書いた
数は間違っていると思え」**のとおり、Velour は 580 → 528、Sparkleur の図は
236 → 176 → 200 と動いている。**UI の欠陥はテストが 1 本も落ちない。**

**バックエンドは nih-plug のまま**に決まった（`AIR-4`）。理由は
`LICENSING.md` — 移行は Air 1 本では終わらず、`nxe-ui` と gallery と既存
3 本の UI が全部巻き添えになる。
...
### 1. **実機で窓を見る**（`AIR-13` の前に）

```bash
mise run install air     # bundle して ~/Library/Audio/Plug-Ins へ
mise run gallery         # DotField だけ単体で見るならこちら
```

**寸法はホストの中でしか決まらない**（`.agents/rules/ui.md`）。
720 × 546 は算術で出した数で、実測ではない。見るところ:

- **点のスペクトラムが読めるか。** 32 列 × 16 行で、`BLEND` を倍音側へ振ると
  点が縦に揃い、ノイズ側で散る。**列が 32 では粗すぎるかもしれない** —
  `analysis.rs` の `BANDS` を 48 にすると細かくなり、CPU は 1 本あたり
  15 → 22 µs 程度増える（予算には余裕がある）
- **図の高さ 200 が Air に合っているか。** 3 本で共有している数だが、
  Sparkleur は 236 → 176 → 200 と 2 回動かしている
- **読み値の帯が 7 セルで詰まりすぎていないか**（IN / OUT / GUARD / WIDTH /
  ENV / BRT / TRN）
- **`.class("root")` が効いているか。** 抜けていると窓の地がホストの黒
  （0x00）のまま、パネルだけ 0x0A で明るい箱に見える。**スクリーンショットの
  画素を測るのが一番速い切り分け**

### 2. `AIR-13` — 既定値と耳

**測れるものは全部測ってある。** 残っているのは `dsp.md` の「耳で詰める定数」
の表で **耳** と書いてあるものだけ:

| 何 | 今の値 |
|---|---|
| `BASE_CORNER_HZ` | 3 000 Hz |
| `TILT_MAX_DB` | 12 dB |
| `GRAIN_SECONDS` | 6 ms |
| `H_SOFT` / `H_HARD` | 0.10 / 0.90 |
| `shaper` の `k` | 4.0（折り返しに 10 dB 余っている） |
| `ENV_FLOOR_DB` / `RANGE` | −48 / 36 dB |
| 既定値 7 本 | `SURFACE` 0.35 / `BLEND` 0.50 / `CHARACTER` 0.35 / `FOCUS` 0 / `WIDTH` 0.60 / `FOLLOW` 0.50 / `MIX` 1.00 |

**聴く先**（構想の用途表）: ボーカルチョップ、スネア / クラップ、パッド、
ダブステップベース、ドラムバス。

**`SURFACE` を 1 本回すだけで判断できる**のが最終の受入条件
（`REQ-AIR-022`）。プリセットが無いので既定値が製品の顔。

### 3. その後 — 出荷と次の 1 本

Air が出たら次は **Vocal Depth**（`implementation/roadmap.md`）。
初期反射を作る本で、Glue と Impact に貸す。
