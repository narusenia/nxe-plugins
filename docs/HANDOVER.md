...
**2026-08-27 時点。3 つとも `v0.1.1` で公開済み。テスト 414 本。**

**Air のコアが `AIR-3` まで通っている。** `dsp.md` は書けていて、
`AIR-1`（ゲート）/ `AIR-2`（倍音層）/ `AIR-3`（層のまとめ）が実測付きで済み。
**次は `AIR-4`（ラッパ。ここで音が出る）で、その前に決めることが 1 つある** —
nih-plug のままか nice-plug へ移るか。詳しくは「次にやること」。

出荷済み 3 本に手を入れる用事は無い。残っているのは Sparkleur の既定値の
**主観サインオフだけ**（下のチェックリスト。**リリースの阻害要因ではない** —
既定値の変更は既存セッションを壊さない）。

## Air のコアで分かったこと（`AIR-1`〜`AIR-3`）
...
| `plugins/air/air-core` | **`AIR-1`〜`AIR-3` まで。** ノイズ層（生成・傾き・粒・`WIDTH`）、倍音層、2 系統のまとめ。テスト 36 本 |
...
- **`air-core` はフレームワークを知らない。** 駄目でも戻すのはラッパだけで、
  DSP とテスト 36 本はそのまま

出荷済み 3 本を載せ替えるなら、その前に **`VST3_CLASS_ID` と状態の
シリアライズ形式が一致するか**を測る。ズレたら既存セッションが全滅する。

### 2. `AIR-4` — ラッパとパラメータ（ここで音が出る）

計画は [`../plugins/air/docs/implementation/air-plan.md`](../plugins/air/docs/implementation/air-plan.md)。
**UI はまだ持たず、ホストの既定パラメータ UI で音を確認する**（`VEL-5` と
同じ）。

**`SURFACE` の写像がここで決まる。** 層のトリムは 2 系統を揃えるためだけの
もので、**絶対量は決めていない**（`dsp.md`）。ピンクノイズ −18 dBFS に対して
層が −31 dBFS で出るので、`SURFACE` = 1 が何 dB になるかはここで置く。

**群遅延を測るのもここ。** 倍音層だけがオーバーサンプラを通るので、原音と
足し合わせたときに可聴な位相差が出ないことを測って記録する
（`REQ-AIR-012`）。

### 3. `AIR-5` — Follow Engine

**`Power` を `nxe-audio` に上げる単位でもある。** パワーフォロワの実装が
既に 2 つある（`guard::Follower` と `sparkle::Power`、どちらも private）。
Air が 3 つ目なので `architecture.md` の「3 個目の客が来たら上げる」に当たる。
**時定数は上げない** — 秒で受け取るだけにして値は呼び出し側に残す（`SPK-1`）。

**`TRANSIENT` は検出帯に通してから比を取る**と決めてある（`dsp.md`）。
