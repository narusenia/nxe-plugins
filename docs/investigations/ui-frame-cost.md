...
Studio One で実測した数字で、推測ではない。原因は 1 つではなく 8 つあった。

**そして CPU の合計は、実は本題ではなかった。** 効いていたのは
「**1 フレームがホストのラン ループを何ミリ秒止めるか**」で、それを最後に測る
までこの調査は 3 回外した。
...
**対策**（`narusenia/vizia` + `narusenia/baseview` `nxe-2026-08-30c`）:
**シングルバッファのコンテキストにした。** swap が無くなり `glFlush` だけになる。
...
**13.2 ms → 0.22 ms。** 失うのはティアリングしない保証で、それを払えるのは
原因 5 のおかげ — **変わった箱しか描かない**ので、ティアリングが起きる場所が
メーターか読み値の中しかない。

**`NSOpenGLPFABackingStore` は犯人ではなかった**（外すと swap はむしろ遅くなる:
p50 0.6 → 1.7 ms）。シングルバッファなら 1 枚しか無いので、部分描画は自動的に
成立する。
