...
Studio One で実測した数字で、推測ではない。原因は 1 つではなく 7 つあり、
7 つとも直した。**最後の 2 つが本命**で、それが「窓全部を毎フレーム描き直して
いた」こと。
...
1. `mise run gallery` を開いて、**絵が今までどおりか目で見る**。ここが一番
   大事で、**部分再描画は間違えると「描き残し」という形で出る** — スクロール
   したとき、ホバーしたとき、窓をリサイズしたときに古い絵が残らないか。
   `NXE_GALLERY_HZ=30` で動かしたまま操作すると出やすい。
