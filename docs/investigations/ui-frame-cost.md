...
Studio One で実測した数字で、推測ではない。原因は 1 つではなく 5 つあり、
5 つとも直した。
...
| **対策後**（両フォーク `nxe-2026-08-30`） | **14.4 / 14.3 / 14.0 %** |

**フォークだけで −37 %。** アイドルの窓は 0.9 % のまま。

内訳としては、vizia の描画パッチが 22.6 → 16.2 %、タイマーが 16.2 → 14.3 %。

**タイマーを 30 Hz にすれば 9 % まで落ちる**が、それだとメーターがつっかえる
（原因 3）。5 ポイントは払う価値がある。
