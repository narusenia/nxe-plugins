...
（UP / DOWN / GAIN / SOLO × 5）とグローバル 6 個。テスト 349 本。
...
**`SPK-18` で測れるものを測った**（2026-08-27）。「耳で詰める定数」の半分は
...
テスト 329 本。**UI 側の `cx.spawn` は `SPK-12` から**（まだエディタが無い）。
...
| ID | 単位 | 計画 |
|---|---|---|
| SPK-18 | **既定値と耳** — 進行中。測れるものは固定済み、残りは実素材で 1 周 | `sparkleur-plan.md` |
...
| SPK-18 | 既定値と耳 | 🔨 測定分は済み。**実素材で 1 周が残り** |
