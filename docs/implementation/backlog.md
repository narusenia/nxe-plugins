...
| ID | 単位 | 状態 |
|---|---|---|
| AIR-1 | ノイズ層と `WIDTH`（**ゲート**）。`air-core` もここで作った | ✅ **ゲート通過** `31f363f` |
...
| PUM-2 | OLA バッファと窓（`pumice-core` に自作）。`realfft` の `process()` が確保する罠を踏んだ | ✅ **再構成 −120 dB 以下**（3 レート × 3 段）、ブロックサイズ 1/64/512/4096 で一致、テスト 8 本 |
...
| PUM-4 | **適応 WHERE と `MODE`**（**ゲート**） | ❌ **`PUM-10c` で v1 から落とした。** ゲートは通っていたが、**非現実的に持続的な素材で校正していた** — 実素材では成立しない。3 つの統計量の測定値は `REQ-PUM-003` |
...
| PUM-10 | 窓。**図は全幅、下を左右分割**（罫線で、反転面は増やさない）。色相 118（モス）を追加。ノードのパラメータを「軸上の位置」にした | 🟡 実機で見た結果 4 つ直した（下）。**もう一度見るのが未了** |
| PUM-10b | **実機で見て出た 4 つ** — 読み値の帯が status バーと重複 / 重みが 300 Hz 以下で階段（ビン経由で描いていた）/ **スペクトルが変換のスケールで割られておらず +34 dB**（何を再生しても上端に張り付く）/ **幅のグリップがホバー時のみで発見できない** | ✅ 帯を外した、重みは `nodes::weight_at` を直接標本化、`display::full_scale` で正規化、幅の横棒を常時表示 |
| PUM-10e | **図の階段と棘**（どちらも実機で発見）。表示格子は低域でビンより細かい — 平均すると同じビンが複数点に返って**階段**。丸めたビン添字で分岐すると格子がビン境界を跨ぐたびに挙動が入れ替わって**棘** | ✅ 連続量で切り替え、境目は混ぜる、DC は読まない。「滑らかな入力が滑らかに出る」をテストで固定 |
| PUM-10d | **実機で見て出たレイアウト 4 つ** — ノブが `Auto` 幅で 7 つの無関係な間隔に並ぶ / `L N H` が読めない / `DELTA` が何にも属さない / **`QUALITY` のラベルが `ink-muted` で暗い地に暗い字**（反転面用のクラスをパネルに使った）。あわせて**図を 320 に**（ラインで唯一 200 でない。理由は `ui.md`） | ✅ |
| PUM-10c | **`LOW` / `HIGH` が図に追随しない**（`Poll` から `sync_nodes()` を落としていた）/ **`ADAPTIVE` が実素材で何も削らない** | ✅ 追随を直した。**`ADAPTIVE` は v1 から落とした** — 3 つの統計量を測って 3 つとも分離しない（`REQ-PUM-003`）。代役に **`THRESHOLD`** を出し、図に**縦軸（dB）**を足した |
