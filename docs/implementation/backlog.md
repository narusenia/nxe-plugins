...
| SPK-17 | **CPU 予算** — criterion。4x / 512 サンプル / 48 kHz | `sparkleur-plan.md` |
...
| デノーマルの掃除 | `SPK-16` で Sparkle のゲートだけ直した。検波器と Guard のエネルギーも 1 次で 0 に漸近するので同じ形が残っている。**`SPK-17` の実測で効いていれば**直す |
