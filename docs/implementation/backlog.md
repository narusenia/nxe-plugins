...
残りは **`VEL-16`（CPU 予算。まだ測っていない）**と
**`VEL-17`（実機で見て・聴いて詰める）**だけ。
...
| ID | 単位 | 計画 |
|---|---|---|
| VEL-16 | CPU 予算の確認（**Velour は未測定**。解析 2 本 × 48 バンドの値が要る） | `velour-plan.md` |
| VEL-17 | 実機で見て・聴いて詰める（**目と耳が要る**） | `velour-plan.md` |
