...
**既定値と読み値がぶつかっているのを見つけた。** `CHARACTER` の既定 0.27 は
`REQ-SPK-006` が「POLISH 寄り」として決めた範囲内だが、`ui.md` の読み値規則
「一番近いアンカー名 + %」では **GLOSS のほうが近い**（0.23 対 0.27）ので
「GLOSS 27 %」と出る。どちらも仕様どおりで、別々に決められたためにぶつかった。
**`SPK-18` でどちらを動かすか決める。**
