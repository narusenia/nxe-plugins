...
| PUM-10c | **`LOW` / `HIGH` が図に追随しない**（`Poll` から `sync_nodes()` を落としていた）。**`ADAPTIVE` が構造的に `STATIC` より弱い** — `min(WHEN, WHERE)` が平均を削減量の上限にしていた | ✅ 地図をゲートに変えた。**共鳴では `ADAPTIVE` = `STATIC`（−23.8 / −23.9）、歌の倍音には 0.00 dB**。`SLOPE` 3.0 / `CEILING` 24 |
