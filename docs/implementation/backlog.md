...
| ID | 単位 | 計画 |
|---|---|---|
| INFRA-1 | ワークスペース骨格（空のクレート 3 つ + xtask がバンドルまで通る） | `infra-plan.md` |
| DBL-1 | リングバッファ遅延線と Hermite 補間 | `doubler-plan.md` |
...
|---|---|---|
| INFRA-1 | ワークスペース骨格 | 🟡 |
| INFRA-2 | プルリクエストの CI（`mise run check`） | ⬜ INFRA-1 |
...
| ID | 単位 | 状態 |
|---|---|---|
| UI-1 | クレート骨格・テーマトークン・gallery が起動する | ⬜ INFRA-1 |
...
| ID | 単位 | 状態 |
|---|---|---|
| DBL-1 | リングバッファ遅延線と Hermite 補間 | 🟡 |
| DBL-2 | 2 タップクロスフェードピッチシフタ（**ゲート**: 狙う音になるか） | ⬜ DBL-1 |
| DBL-3 | ボイスエンジン（形状表・実効値・パン則・ゲイン補償） | ⬜ DBL-2 |
