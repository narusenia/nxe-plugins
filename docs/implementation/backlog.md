...
| PUM-4b | **「効きが弱い」を測った**（実機の感想から生えた単位）。仕様の誤り 4 つ — 参照が自分を含む / 1 ビンの分散 / 地図が無音を平均 / `SLOPE` の二重上限。2 度目の感想「削りすぎまで行かない」で `SLOPE` を 2.0 に | ✅ 同じ共鳴への削減 **3.4 → 15.6〜17.1 dB**、同時に普通の素材への削減 **0.71 → 0.01 dB** |
