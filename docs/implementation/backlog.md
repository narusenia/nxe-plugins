...
| ID | 単位 | 計画 |
|---|---|---|
| DBL-2 | 2 タップクロスフェードピッチシフタ（**ゲート**） | `doubler-plan.md` |
| UI-1 | テーマトークンと gallery の中身 | `nxe-ui-plan.md` |
| INFRA-2 | プルリクエストの CI | `infra-plan.md` |

順序は [`roadmap.md`](roadmap.md) が決める。今は `DBL-2`（フェーズ 1 のゲート）が
次。`UI-1` はフェーズ 3、`INFRA-2` はフェーズ 4。
...
| DBL-2 | 2 タップクロスフェードピッチシフタ（**ゲート**: 狙う音になるか） | 🟡 |
