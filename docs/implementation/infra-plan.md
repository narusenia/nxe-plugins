...
- **決めたこと**: **タグは `<plugin>-v<version>`**（`doubler-v0.1.0`）。
  プラグインごとに独立して出す — 1 個直しただけで全部のバージョンが上がり、
  変わっていないものまで再リリースされるのを避ける。`AGENTS.md` の
  「プラグインは単体リポジトリに切り出せる」とも一致する。したがって
  **バージョンはワークスペースではなくプラグインの Cargo.toml が持つ**
  （`doubler-core` は同じ出荷単位なので一緒に動く）
- **決めたこと**: タグから**プラグイン名とバージョンの両方を読む**。
  ワークフローに `doubler` を書き込まないので、2 個目が来ても触らずに済む
- **決めたこと**: **タグとクレートのバージョンが違ったら落とす。** 食い違うと
  「正しい名前で間違ったものを出荷する」ことになり、成果物は正しく見える
