...
ことは一度も無い。** Pumice は `3N/4`（48 kHz `NORMAL` で 32 ms）を申告する。
...
副次的に、自前のほうが**申告レイテンシが短い** — `StftHelper` の
`latency_samples()` はブロック長 `N` そのもので、`N − hop` ではない。
**答え合わせの相手としては使う**（`PUM-2` の完了条件）。
